describe 'game app', ->
  ...
  describe 'App', ->
    describe 'dealer', ->
      ...
      it "should end game if its value is more than 17", -> #fails sometimes for sheer random?
        spyOn app, 'gameEnd'
        app.get('playerHand').trigger 'stand'
   ...
   describe 'score', ->
      it "should let the dealer win if their hand value is greater or equal to player hand value", ->
        spyOn app, 'trigger("playerWin")'
        app.set 'dealerHand', new Hand [ new Card( rank: 1, suit: 1 ), new Card( rank: 0, suit: 2 ) ], app.get 'deck'
        app.set 'playerHand', new Hand [ new Card( rank: 0, suit: 1 ), new Card( rank: 0, suit: 3 ) ], app.get 'deck'
        app.get('playerHand').trigger 'stand'
        expect(app.trigger 'playerWin').toHaveBeenCalled()
